...
2. **Issue ID** is unique identifier for an issue and is generated by ppp when issue is created. It  not only reference an issue in other ppp features (sprint tracking) but also generate markdown files name and folder names for the types of issue prompt documents. Here are the Issue ID generation rules by Issue Types:
  ...
  - **Rules of Issue ID Generation**
    ...
    2. **Module/Feature ID**: Modules or Features are fixed to be in 3 layers:
      - layer 1 module IDs gos with prefix plus 2-fixed-digits like F01, F02, ... F99. The F prefix means module/feature, the 2-digits means layer 1 modules (the top module).
      - layer 2 module/feature IDs gos with prefix plus 4-fixed-digits like F0101, F0101, .. F0199, F1201, F1202, ...F1299. The F prefix means module/feature, the first 2 digits is an internal ID part that means it is under the layer 1 modules (the top module), the last 2 digits is an internal ID part in it parent module (layer 1 module).
      - layer 3 feature IDs gos with prefix plus 6-fixed-digits like F010101, F010102, ...F010199, F010105, F120201. The F prefix means module/feature, the first 2 digits means it is under a specifc layer 1 module, the second 2-digits is an internal ID part that means it is under a specific layer 2 module, the last 2 digits is an internal ID part in its parent layer 3 module.

    3. **Epic/User Story/Task ID**: The type of issues must be put under a specific module/feature doc folder and ID must be prefixed with a specific feature ID's digits part, and also be suffixed with an auto-incremented 2-digits number. i.e. if a user story belongs to a feature F0105, The user story ID goes like T010501, "0105" mean the user story belongs to feature F0105. if a user story belongs to module F01, The user story ID goes like T0101, "01" mean the user story belongs to module F01.

    4. **Bug ID**: Bugs must be put under a specific module/feature doc folder and ID must be prefixed with a specific feature/Epic/User Story/Task ID's digits part, and also be suffixed with an auto-incremented 2-digits number as what Epic/User Story/Task ID does. i.e. if a bug belongs to a feature F0105, The bug ID goes like B010501, "0105" mean the bug belongs to feature B0105. if a bug belongs to feature T010101, The bug ID goes like B010101, "01" mean the bug belongs to module F0101.
...
4. **Issue File** is the file that contains all the issue related info. It is created when issue is created and managed by ppp during issue lifetime.
- Modules/Features: Its path is like `.ppp/<issue_container_folder>/spec.md`. i.e. `F01_admin/spec.md`, `F01_admin/F02_user_management/spec.md`, etc. It should includes all the issue attributes and other details of requrment and implemation in the md file.
- Epics/User Stories/Tasks: Its path is like `.ppp/<module_folder>/<feature_folder>/issue_file.md`. i.e. `F01_admin/T01_create_admin_ddl.md`, `F01_admin/F01_user_management/T03_add_user.md` and `F01_admin/F01_user_management/F02_add_user/F05_check_username.md`. It should includes all the issue attributes and other details related the issue type in the md file.
- Bugs: Its path is like `.ppp/<module_folder>/<feature_or_task_folder>/bug_file.md`. i.e. `F01_admin/F01_user_management/B03_cannot_load_module_page.md` and `F01_admin/F01_user_management/F02_add_user/B05_fail_to_submit.md`. It should includes all the issue attributes and bug details in the md file.

5. **Issue Container Folder** is the (modules/features) folder that contains all the child issues related to the module/feature. It is created when module/feature is created and managed by ppp during module/feature lifetime. Its path is like `.ppp/<issue_container_folder>`. i.e. `F01_admin`, `F01_admin/F02_user_management`, etc.
...
### Feature Bill
The **Feature Bill** serves as a hierarchical, structured index of all modules and features within the product, organizing them by their nested layers to provide a clear overview of the product’s functional architecture. It maps out the relationships between modules (Layer 1), sub-modules/features (Layer 2), and granular features (Layer 3), aligning with the issue ID rules and folder structure defined for PPP.

#### **Purpose**
The Feature Bill enables teams to:
- Visualize the full hierarchy of the product’s modules and features.
- Quickly locate specific modules/features and their associated details (e.g., status, assignee, description).
- Understand parent-child relationships between nested layers (e.g., which Layer 2 features belong to a Layer 1 module).
...
The Feature Bill strictly follows the 3-layer structure of modules and features, as defined by their Issue IDs and corresponding folder paths:

##### **1. Layer 1 Modules**
- **Definition**: Top-level modules that form the foundational structure of the product.
...
- **Associated File**: Each Layer 1 module contains a `spec.md` file at `.ppp/F<XX>-<keywords>/spec.md`, which documents its attributes (ID, status, priority, description, etc.).


##### **2. Layer 2 Modules/Features**
- **Definition**: Sub-modules or features nested under a specific Layer 1 module, refining its functionality.
- **ID Format**: `F` + 4 fixed digits (first 2 digits = parent Layer 1 module ID; last 2 digits = unique identifier for the Layer 2 item).
  Example: `F0102` (nested under Layer 1 module `F01`).
- **Folder Path**: Nested within the parent Layer 1 module’s folder:
...
##### **3. Layer 3 Features**
- **Definition**: Granular features nested under a specific Layer 2 module, representing detailed functional components.
- **ID Format**: `F` + 6 fixed digits (first 2 digits = parent Layer 1 module ID; middle 2 digits = parent Layer 2 module ID; last 2 digits = unique identifier for the Layer 3 feature).
  Example: `F010203` (nested under Layer 2 module `F0102`, which is under Layer 1 module `F01`).
- **Folder Path**: Nested within the parent Layer 2 module’s folder:
...
- Epics, user stories, and tasks (with IDs prefixed by `T`) are nested in the folders of their parent modules/features (e.g., a task `T010201` lives in `.ppp/F01-<keywords>/F02-<keywords>/`).
- Bugs (with IDs prefixed by `B`) are similarly nested under their associated modules/features or tasks.
...
- issue_type is requred and must be one of `module`, `feature`, `epic`, `story`, `task`, `bug`.
...
During **issue creation**, ppp will:
  ...
  - update Feature Bill if the created issue is a module/feature.

**Naming Convention of Issue files/folders**
Folders for modules and features follow a hierarchical numbering system, with each layer nested under the previous one.
  1. ** Modules/Features (Folders Only)**
    - **Layer 1 Modules**:  Located directly under the `.ppp` folder, named s:  `F01-<issue_name_keywords>`, `F02-<issue_name_keywords>`, ..., F12-<issue_name_keywords>`
    - **Layer 2 Modules/Features**:  Nested under a specific Layer 1 module folder, named as: `F01-<issue_name_keywords>`, F02-<issue_name_keywords>`, ... (e.g., A Layer 2 module "F0102" would be stored at `.ppp/F01-<issue_name_keywords>/F02-<issue_name_keywords>`)
    - **Layer 3 Features**: Nested under a specific Layer 2 module folder, named as: `F01-<issue_name_keywords>`, `F02-<issue_name_keywords>`, ... (e.g., A Layer 3 feature "F010201" would be stored at `.ppp/F01-<issue_name_keywords>/F02-<issue_name_keywords>/F01-<issue_name_keywords>`)

  2. **Epics/User Stories/Tasks (markdown files only)**
Epics, user stories, and tasks are created as `.md` files, nested under the corresponding module/feature folder, with the following naming format: `T01-<issue_name_keywords>.md`, `T02-<issue_name_keywords>.md`, ...
  - Example 1: A task "T010201" would be stored at:
  `.ppp/F01-<issue_name_keywords>/F02-<issue_name_keywords>/T01-<issue_name_keywords>.md`
  - Example 2: A Epic "T020502" would be stored at:
    `.ppp/F02-<issue_name_keywords>/F05-<issue_name_keywords>/T02-<issue_name_keywords>.md`
  - Example 3: A Bug "B01020111" would be stored at:
  `.ppp/F01-<issue_name_keywords>/F02-<issue_name_keywords>/F01-<issue_name_keywords>/T11-<issue_name_keywords>.md`
